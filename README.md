...
## Data & Documentation
Due to some data in this dataset provided by the Ohio Department of Natural Resources, Cleveland Metroparks, Purdue University, and Ohio University not being public, only part of the original dataset used for these analyses has been added to this repo. The full dataset may be available upon request. Running this analysis with a partial dataset _will_ produce differing results. Datasets Filtered for only public data were the same between all iterations.

Information about the data tables can be found in `oh-in-weasel-dist-model_datadict.json`, which contains metadata for each field formatted as JSON.
